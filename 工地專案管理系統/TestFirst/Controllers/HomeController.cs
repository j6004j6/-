...
namespace TestFirst.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ContentResult AJAX_Home(string id)
        {
            ...
            {
                int done = db.tMission_SignOff.Where(p => p.fStatus == "已駁回").Select(p => p).Count();
                string date = DateTime.Now.ToString("yyyyMMdd");
                int todo = db.tMission.Where(p => p.fEST_Time ==date ).Select(p=>p).Count();

                Notification = $"今日有{todo}個工程要進行,有 {done} 個申請被駁回";
                return Content(Notification);
            }
            return Content(Notification);
        }
        ...
        public ActionResult HumanResource()
        {
            if (Session[CDictionary.SK_CURRENT_LOGINED_USER] != null)
            {
                if (Session["UserId"].ToString().Substring(0, 1) == "4")
                {
                    ...
                    var q = db.tProject_HumanCost.OrderBy(p=>p.fProjectCode).Select(p => p).ToList();
        ...
        // 照片功能>>>新增照片
        public ActionResult Photos_new(string name="")
        {
            ViewBag.Project = db.tProject.Select(p => p).ToList();
            ViewBag.fname = name;
            return View();
        ...
        // AJAX_取消新增回到日報列表
        public ActionResult Photos_AJAX(string name="")
        {
            if (name != "")
            {
                var q = db.tMissionDetail.Where(p => p.fName == name).Select(p => p.fCode).ToList();
                if (q.Count != 0)
                {
                    if (q[0]!=null)
                    {
        ...
        // 照片功能>>>刪除照片
        public ContentResult Photos_delete(string id)
        {
            try
            {
                id = id.Substring(4);
                var q = db.tMissionDetail_Photos.Where(p => p.fId.ToString() == id).FirstOrDefault();
                db.tMissionDetail_Photos.Remove(q);
                db.SaveChanges();
                return Content($"成功刪除{id}");
            }
            catch (Exception e)
            {
        ...
        // 公告與留言區
        public ActionResult 留言板(string order="")
        {
            ...
            else
            {
                ...
                if (order == "dep")
                {
                    m = m.OrderBy(p=>p.fdepartment);
                }
                else if (order == "date")
                {
                    m = m.OrderBy(p => p.fdatetime);
                }
        ...
        public ActionResult 信箱()
        {
            ...
            else
            {
                ...
                var mail = from m in db.tmail
                           where m.fsendid.ToString() == id || m.frecieveid.ToString() == id
                           orderby m.fsendid descending
                           select m;
                int count = 0;
                var mail1 = from tm in db.tmail
                            where tm.frecieveid.ToString() == id
                            select tm;
        ...
        public ActionResult SendMail(tmail tmail)
        {
            ...
            t.fsendid = int.Parse(u.fEmployeeId);
            db.tmail.Add(t);
            db.SaveChanges();
            return RedirectToAction("信箱");
        }
        public ActionResult GetUser_Mail(string name) 
        {
            var q = db.tUser.Where(p => p.fEmployeeId.Contains(name)).Select(p => p.fEmployeeId).ToList();
        ...
        public ActionResult ReplyMail(tmail tmail)
        {
            ...
            t.fsendid =Convert.ToInt32( t1.frecieveid);
